...
**Status as of 2026-05-02:** PRs 1, 2, and 3 committed locally on `master` (not pushed). Ready to start PR 4.
...
| 3 | Switch entry point, delete HTML pipeline, rename plugin, update build + `deploy.yml` (step 5) | 🟢 Done (local) | (see git log) |
| 4 | README, schema, examples, migration note | ⏳ Not started | — |

**Uncommitted in working tree right now:** none after this commit lands.

**Next concrete action:** start PR 4 — rewrite `README.md` to describe the Markdown-report plugin (install, config, env vars), update `schema.json` to drop the deleted env vars (`gauge_minify_reports`, theme path) and bump the schema version, and add a migration note for the v4 → v5 transition (plugin id rename, output-format change, removed env vars). Optionally land an example output snippet generated by running the integration test fixtures. Step 6 (manual run against a real Gauge project) cannot be done by Claude — mark as a user task in the PR description.
